...
import { providerCreateSchema } from "@/lib/validators/provider";
...
export async function GET(request: NextRequest) {
  ...
  const { data, count, error: queryError } = await context.supabase
    .from("providers")
    .select("id, user_id, specialty, availability, created_at", { count: "exact" })
...
export async function POST(request: NextRequest) {
  ...
  if (!hasRequiredRole(context.role, ["admin", "staff"])) {
    return error(403, "forbidden", "Insufficient role permissions");
  }
  ...
  if (insertError || !data) {
    return error(500, "insert_failed", insertError?.message ?? "Unable to create provider");
  }
