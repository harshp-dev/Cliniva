...
import { carePlanCreateSchema } from "@/lib/validators/care-plan";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = carePlanCreateSchema.safeParse(payload);
  if (!parsed.success) {
  ...
  if (insertError || !data) {
    return error(500, "insert_failed", insertError?.message ?? "Unable to create care plan");
  }
